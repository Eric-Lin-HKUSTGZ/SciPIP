...
def extract_ideas(idea_str):
    if idea_str is None:
        return []  # 返回空列表而不是空字符串，保持返回类型一致
    if not isinstance(idea_str, str) or len(idea_str.strip()) == 0:
        return []  # 处理空字符串或其他无效输入
    ideas = []
    for i in range(1, 100): # 100 is a magic number
        ...
        if start_index != -1 and end_index != -1:
            ...
            break
    return ideas if ideas else [idea_str]  # 如果找不到格式化的 idea，返回包含原始字符串的列表
...
class IdeaGenerator:
    ...
    def generate_ins_bs(self, detail_background: str):
        ...
        def generate_inspiration(paper, i):
            ...
            # 检查 detail_method 是否为 None
            if detail_method is None:
                logger.warning(f"Generate concise method for related paper {i} failed (returned None)")
                return
            
            inspiration = self.api_helper.generate_inspiration_with_detail_method(detail_background, detail_method)
            # 检查 inspiration 是否为 None
            if inspiration is None:
                logger.warning(f"Generate inspiration for related paper {i} failed (returned None)")
                return
        ...
        logger.info(f"Generate inspirations for all related papers succeed. Total inspirations: {len(inspirations)}")
        
        # 检查 inspirations 是否为空
        if len(inspirations) == 0:
            logger.warning("No inspirations generated (all API calls may have failed). Cannot generate ideas.")
            initial_ideas = []
            idea_filtered = None
            final_ideas = []
            return None, None, inspirations, initial_ideas, [], final_ideas
        
        idea = self.api_helper.generate_idea_by_inspiration(detail_background, inspirations)
        if idea is None:
            logger.error("Failed to generate ideas from inspirations (API call returned None)")
            initial_ideas = []
            idea_filtered = None
            final_ideas = []
            return None, None, inspirations, initial_ideas, [], final_ideas
        ...
        logger.info(f"Generate ideas from inspirations succeed. Extracted {len(initial_ideas)} initial ideas")
        
        idea_filtered = self.api_helper.integrate_idea(detail_background, self.brainstorm, idea)
        if idea_filtered is None:
            logger.error("Failed to integrate ideas (API call returned None). Using initial ideas as final ideas.")
            final_ideas = initial_ideas
            ideas_filtered = initial_ideas
        ...
        logger.info(f"Extracted {len(ideas_filtered)} ideas after filtering")
        
        if len(ideas_filtered) == 0:
            logger.warning("No ideas extracted after filtering. Returning empty final ideas list.")
            final_ideas = []
        else:
            final_ideas = ["None"] * len(ideas_filtered)
        def expand_idea(detail_background: str, idea: str, i):
            try:
                if idea is None or (isinstance(idea, str) and len(idea.strip()) == 0):
                    logger.warning(f"Expand the {i}th idea skipped: idea is empty or None")
                    final_ideas[i] = "Failed to expand: idea is empty"
                    return
                
                expanded = self.api_helper.expand_idea(detail_background, idea)
                if expanded is None or (isinstance(expanded, str) and len(expanded.strip()) == 0):
                    logger.warning(f"Expand the {i}th idea failed: API returned None or empty")
                    final_ideas[i] = f"Failed to expand idea (API timeout or error). Original idea: {idea[:100]}..." if len(idea) > 100 else f"Failed to expand idea (API timeout or error). Original idea: {idea}"
                else:
                    final_ideas[i] = expanded
                    logger.info(f"Expand the {i}th idea succeed")
            except Exception as e:
                logger.error(f"Error expanding the {i}th idea: {type(e).__name__}: {e}")
                final_ideas[i] = f"Error expanding idea: {str(e)[:100]}..." if len(str(e)) > 100 else f"Error expanding idea: {str(e)}"
        processes = []
        for i, idea in enumerate(ideas_filtered):
            p = threading.Thread(target=expand_idea, args=(detail_background, idea, i))
            processes.append(p)
            p.start()
        # 增加等待时间，因为每个请求可能有重试（最多3次，每次120秒）
        # 但为了不无限等待，设置一个合理的上限（如 480 秒 = 8 分钟）
        for p in processes:
            p.join(480)  # 增加等待时间到 8 分钟，适应 API 重试机制
