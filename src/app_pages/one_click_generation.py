...
def generate_ideas(backend, background):
    ...
    with st.spinner(text=("Understanding the user's input...")):
        try:
            ...
            if expanded_background is None:
                st.error("⚠️ Failed to expand background. This may be due to:")
                st.error("• API server timeout (response took too long)")
                st.error("• Network connection issues")
                st.error("• API server is unavailable")
                st.info("💡 Please check your network connection and API server status, then try again.")
                return
        except Exception as e:
            st.error(f"⚠️ Unexpected error when expanding background: {str(e)}")
            st.info("💡 Please check your API configuration and network connection.")
            return
    ...
    with st.spinner(text=("Brainstorming...")):
        ...
        if brainstorms is None:
            st.error("Failed to generate brainstorm. Please check your API connection and try again.")
            return
    ...
    with st.spinner(text="Generating ideas... (This may take up to 5 minutes)"):
        ...
        for i in range(len(initial_ideas)):
            # 处理 final_ideas[i] 为 None 或空的情况
            if i < len(final_ideas) and final_ideas[i] is not None:
                detail_content = final_ideas[i]
            else:
                detail_content = "*Unable to generate detailed idea due to API timeout or error. Please try again later or check your API connection.*"
