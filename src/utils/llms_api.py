...
class APIHelper(object):
    ...
    def expand_idea(self, background: str, idea: str) -> str:
        ...
        if idea is None or (isinstance(idea, str) and len(idea.strip()) == 0):
            print("Input idea is empty or None ...")
            return None
        ...
        try:
            ...
            # 检查响应是否为空
            if detail_ideas is None or (isinstance(detail_ideas, str) and len(detail_ideas.strip()) == 0):
                print("Warning: expand_idea returned empty response")
                return None

        except requests.exceptions.ReadTimeout as e:
            print(f"Error: API request timeout when expanding idea: {e}")
            traceback.print_exc()
            return None
        except requests.exceptions.ConnectionError as e:
            print(f"Error: Cannot connect to API server when expanding idea: {e}")
            traceback.print_exc()
            return None
        except Exception as e:
            print(f"Error: Unexpected error when expanding idea: {type(e).__name__}: {e}")
            traceback.print_exc()
            return None
    ...
    def expand_background(self, brief_background: str, keywords: str) -> str:
        ...
        try:
            ...
            # 检查响应是否为空
            if detail_background is None or (isinstance(detail_background, str) and len(detail_background.strip()) == 0):
                print("Warning: expand_background returned empty response")
                return None

        except requests.exceptions.ReadTimeout as e:
            print(f"Error: API request timeout when expanding background: {e}")
            traceback.print_exc()
            return None
        except requests.exceptions.ConnectionError as e:
            print(f"Error: Cannot connect to API server when expanding background: {e}")
            traceback.print_exc()
            return None
        except Exception as e:
            print(f"Error: Unexpected error when expanding background: {type(e).__name__}: {e}")
            traceback.print_exc()
            return None
    ...
    def generate_idea_by_inspiration(self, background: str, inspirations: list[str]):
        ...
        # 检查 inspirations 是否为空列表
        if isinstance(inspirations, list) and len(inspirations) == 0:
            print("Warning: inspirations list is empty. Cannot generate ideas without inspirations.")
            return None
        ...
        try:
            ...
            # 检查响应是否为空
            if idea is None or (isinstance(idea, str) and len(idea.strip()) == 0):
                print(f"Warning: generate_idea_by_inspiration returned empty response")
                return None
        except Exception as e:
            print(f"Error in generate_idea_by_inspiration: {type(e).__name__}: {e}")
            traceback.print_exc()
            return None
        return idea
